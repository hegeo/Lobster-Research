...
JSON 文件命名规范（按数据类型分层）：
  ...
  1_market_akshare_macro.json     AKShare 结构化数据（龙虎榜/北向资金/融资融券）
...
"""
        ...
        if user_prefs:
            ...
            style_raw   = u.get("investment_style", "")
            regime_raw  = m.get("regime", "")
            style       = label("investment_style", style_raw)
            ...
            regime      = label("market_regime", regime_raw)

            # ── 个人风格 vs 市场感知匹配检测 ──
            STYLE_REGIME_WARN = {
                ("growth", "bear"): (
                    "🟥 高风险匹配：成长风格 × 熊市\n"
                    "    成长股在熊市中弹性标的回撤极大（通常30-50%），与用户画像严重冲突。\n"
                    "    建议：仓位降至30%以下，转向高股息/公用事业等防御品种"
                ),
                ("trend", "bear"): (
                    "🟥 高风险匹配：趋势风格 × 熊市\n"
                    "    熊市中趋势持续性差，追高极易站岗。\n"
                    "    建议：暂停新开仓，已有持仓严格执行-5%移动止损"
                ),
                ("band", "bull"): (
                    "🟨 注意：波段风格 × 牛市\n"
                    "    牛市中波段操作容易过早止盈踏空主升浪。\n"
                    "    建议：放宽止盈至20-30%，底仓50%中线持有不动"
                ),
                ("value", "bull"): (
                    "🟨 注意：价值风格 × 牛市\n"
                    "    价值股在牛市中弹性不足，可能跑输大盘。\n"
                ...
                ("trend", "oscillate"): (
                    "🟨 注意：趋势风格 × 震荡市\n"
                    "    震荡市中趋势信号频繁假突破。\n"
                    "    建议：缩小周期至60分钟线，降低仓位至50%"
                ),
            }
            mismatch_key = (style_raw, regime_raw)
            mismatch_warn = STYLE_REGIME_WARN.get(mismatch_key, "")
            mismatch_section = f"\n{mismatch_warn}\n" if mismatch_warn else ""
...
**兴趣与关注领域**: {focus}
**关注市场或标的**: {focus_stk}
{mismatch_section}⚠️ **铁律：你必须根据以上用户画像调整报告内容！**
...
> **⚠️ 给 Agent 的重要提示**
> 本简报即是最终生成提示词，包含完整的用户画像、数据清单和报告结构参考。
> **数据源仅需：** 读取各 JSON 数据文件 + 本简报（截断部分用 `Read offset` 补读）。
> **不需要** 读取 `prompts/` 下任何模板文件，`5_agent_report_input.json` 已预置完整章节骨架，直接在上面填写即可。
...
"""
    ...
    def generate_report(self, agent_input_path: str) -> Tuple[bool, dict]:
        ...
        try:
            ...
            # 注入 meta 的 prompt_template，供质量门禁读取 type/阈值
            if "prompt_template" not in report_data and self.meta.get("prompt_template"):
                report_data["prompt_template"] = self.meta["prompt_template"]
            ...
            # 用 title 字段生成文件名（去空格和特殊符号）
            raw_title = report_data.get("title", f"report_{task_id}")
            safe_title = re.sub(r'[\\/*?:"<>|\s·—–_]+', "_", raw_title)
            safe_title = re.sub(r'_+', "_", safe_title).strip("_")
            filename = f"{safe_title}_{task_id}" if safe_title else f"report_{task_id}"
            ...
            result = generate_report(
                ...
                data=report_data,
                output_format="pdf",
                output_path=os.path.join(self.task_dir, f"{filename}.pdf"),
                style=style,
    ...
    def run_market_flash_news(self) -> Tuple[bool, str]:
        """
        采集大盘行情快讯、盘面热点资讯，写入 3_news_market_flash.json
        调用 akshare_api_kit.py 的 --json 新闻 模式获取结构化数据
        """
        self._log.info("  开始采集市场快讯: 新闻 焦点 + 新闻 A股")
        all_news = []
        keywords = ["A股", "焦点"]
        errors = []
        ...
        for kw in keywords:
            try:
                ok, stdout = self._run_script(
                    ["scripts/akshare_api_kit.py", "--json", "新闻", kw],
                    timeout=30,
                )
                if ok and stdout.strip():
                    import json as _json
                    records = _json.loads(stdout)
                    if isinstance(records, list):
                        for r in records:
                            r["_keyword"] = kw
                        all_news.extend(records)
                        self._log.info(f"    {kw}: {len(records)} 条")
                    else:
                        errors.append(f"{kw}: 返回格式异常")
                else:
                    errors.append(f"{kw}: 脚本执行失败")
            except Exception as e:
                errors.append(f"{kw}: {e}")

        data = {
            "_meta": {
                "step": "news_market_flash",
                "fetched_at": datetime.now().isoformat(),
                "keywords": keywords,
            ...
            "news": all_news,
            "agent_note": "大盘行情快讯、盘面热点资讯，请整合进报告市场分析章节",
        }
        path = self._write_json("3_news_market_flash.json", data)
        ok = len(all_news) > 0
        if not ok:
            self._log.warn(f"  市场快讯采集失败: {errors}")
        return ok, path
    ...
    def _clean_schema_comments(self, obj: Any) -> Any:
        """递归删除 JSON 中所有以 _ 开头的注释字段（保留 _report_type 等程序元数据）"""
        _KEEP_META = frozenset({"_report_type", "_task_id"})
