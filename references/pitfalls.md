...
#### JSON 字符串内未转义的控制字符导致解析失败

**现象**：`5_agent_report_input.json` 在 Phase 3 生成时抛出 `Invalid control character` 错误。常见于 Agent 填写的内容中包含字面换行符（`\n`）、回车符（`\r`）、制表符（`\t`）等控制字符。

**根因**：JSON 标准规定字符串内的控制字符必须转义（`\n` → `\\n`），但 Agent 通过 Write 工具写入时保留了字面换行符。此外，内容中的 ASCII 直引号 `"..."` 也与 JSON 字符串定界符冲突。

**修复（2026-05-12）**：
1. 将 `_fix_json_chinese_quotes()` 升级为 `_fix_json_common_issues()`，新增控制字符转义逻辑
2. 新增状态机遍历每个字符，在 JSON 字符串内部自动将 `\n`→`\\n`、`\r`→`\\r`、`\t`→`\\t`、其他 `\x00-\x1F`→`\\u00xx`
3. 在 `validate_quality.py` 中新增 `auto_fix_json()` 和 `load_json_with_auto_fix()`，可作为独立模块调用
4. 修复链：直接解析 → 综合修复 → 回写文件，失败时保留 `.bak` 备份

**修改文件**：`scripts/task_runner.py:1094-1200`、`scripts/validate_quality.py:107-230`

**预防**：5_agent_report_input.json 填写时确保字符串值内不使用字面换行符和 ASCII 直引号。
...
### 5_agent_report_input.json 缺少 prompt_template 字段导致 generate 崩溃

**现象**：Phase 3 `python main.py generate --task-id XX` 失败，错误依次为：
1. `unhashable type: 'dict'` — `_templates.get(key, ...)` 中 `key` 是整个 data dict
2. 修复为 string 后 → `'str' object has no attribute 'get'` — `check_report_input` 期望 dict 类型

**根因**：`5_agent_report_input.json` 顶层缺失 `prompt_template` 字段（dict 类型）。`_get_chinese_report_name()` 中 `data.get("prompt_template")` 返回 None，触发 `or get_template(data)` 将整个 data dict 传给 `_templates.get(key)`，而 dict 不可哈希。即使 `_clean_schema_comments` 不会删除该字段（不以 `_` 开头），模板填充功能也未自动添加。

**正确的 prompt_template 格式**：
```json
"prompt_template": {"name": "研报-选股研究", "type": "deep", "sections": 12, "min_chars": 5000}
```

**预防**：Phase 2 填写 `5_agent_report_input.json` 时必须在顶层添加 `prompt_template` 字段，格式为包含 `name`、`type`、`sections`、`min_chars` 的 dict。`name` 用于文件名生成，`type` 控制质量门禁模式（deep/normal），`sections`/`min_chars` 为质量阈值。

**发现记录**：2026-05-13 快速选股深度研报生成时首次发现。

**根治修复（2026-05-13）**：
1. 移除 `_get_chinese_report_name()` 中 `or get_template(data)` 的兜底调用——`get_template()` 期望 `str` 类型 key，传入整个 data dict 会导致 `unhashable type: 'dict'`
2. 改为纯 `report_label` 兜底（来自 `REPORT_TYPES[label]`，如"快速选股""大盘日报"等中文名，做文件名前缀绰绰有余）
3. 移除 `get_template` 的 import（该文件内已无其他调用）

**根本原因**：`_get_chinese_report_name` 的设计是"优先从 prompt_template 取 name，否则从 report_label 取"，但其第 2 步 `get_template(data)` 误将整个 report data dict 当作模板键名传入，逻辑错误。

**后续影响**：`prompt_template` 字段在 `5_agent_report_input.json` 中仍可保留（用于质量门禁 `_get_quality_thresholds` 的 `type` 字段判断），但现在**不再是必填项**了——缺失时静默回退到 `report_label` 且不会崩溃。
