...
**`generate_report.py` 现在会在生成后打印质量快报：**

```
✅ [DEEP] 质量快报: 12 sections | 7 tables | 约4200字
⚠️ [QUICK] 质量快报: 4 sections | 0 tables | 约900字 | ⚠️ 2 条警告
```
...
```
generate 完成后看到 ⚠️ 或 ❌ 快报
    ↓
1. 检查具体 warning 内容（缺失字段 / 内容过短 / table格式错误）
2. 修正 5_agent_report_input.json 中的对应问题
3. 重新运行 generate: python main.py generate --task-id <task-id>
4. 直到质量快报显示 ✅，再执行 preview_url + deliver_attachments
```
