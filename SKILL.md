...
* 🆕 新增铁律 8：必须根据用户画像（user_prefs）调整报告内容，禁止万能模板
* 🆕 AGENT_BRIEFING.md 新增「用户画像」章节，从 meta.json.user_prefs 自动注入
* 🆕 phase2_guide.md 新增「第零步：读取用户画像」，含用户类型×风格映射表
* 🔧 task_runner.py: write_agent_briefing() 现在注入投资风格、风险偏好、操作周期、资产规模、关注领域
* 🔧 优化：AGENT_BRIEFING.md 不再重复注入 promptBody 大纲（与 07_agent_input.json 的 _prompt_body 去重），减少 Phase 2 token 消耗
